function ex = OMS_naturalscene(ex, replay)
%
...
% Runs a receptive field mapping stimulus
  ...
  for fi = 1:numframes
    ...
    % pick a new image either from a new image or a different jittering of
    % previous image
    ...
    % get the new frame (or image patch)
    % (defined by an image and start point)
    ...
    if replay
      ...
      % srcRect is not defined. []
      Screen('DrawTexture', ex.disp.winptr, texid, [], ex.disp.dstrect, 0, 0);
      Screen('Close', texid);
      
      % Juyoung add for moving object at the center 10/03 2016
      % 
