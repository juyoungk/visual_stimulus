function [ex, noise_stim] = rf_noise(ex, varargin)
    ...
    randomStream = RandStream('mcg16807', 'Seed', seed);
     ...
     if strcmp(p.Results.recreation, 'yes')
        ifi = ex.disp.ifi;
        framesPerFlip = round( stimFrameInterval/ifi );
        % Nominal-rate-optimized stimulus flip interval (not rate)
        frameTime = ifi * framesPerFlip;
        framesN = uint32( round( DurationSecs / frameTime ));
        %
        noise_stim = RandomCheckersRecreate(framesN, checkersN_V, checkersN_H, objContrast, randomStream, noise);
     else
         try
            ...
            framesN = uint32( round( DurationSecs / frameTime ));
            % 
            ex.stim{end}.params.framesN = framesN;
            %
